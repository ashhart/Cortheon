...
_READ_ONLY_GIT = frozenset({"blame", "diff", "grep", "log", "ls-files", "show", "status"})
_BOUND = 50_000
_WEB_BOUND = 60_000
_OUTPUT_BOUND = 8_000
_MAX_WALK_FILES = 400
...
class OmpHost:
    ...
    def grep(self, pattern: str, path: str = ".", **kwargs: Any) -> _HostResult:
        ...
        if not base.is_relative_to(self.root):
            return _HostResult(
                ...
                "command",
                self._receipt("grep", "error", {"pattern": pattern}),
            )
        searched = 0
        targets = [base] if base.is_file() else sorted(base.rglob("*"))
        for candidate in targets:
            if not candidate.is_file() or searched >= _MAX_WALK_FILES:
                continue
            searched += 1
            try:
    ...
    def shell(self, command: str) -> _HostResult:
        ...
        if not tokens or not self._read_only_tokens(tokens):
            return _HostResult(
                "Only bounded read-only commands are allowed",
                "command",
                self._receipt("bash", "error", {"command": command}),
            )
        try:
            ...
            return _HostResult(
                str(exc), "command", self._receipt("bash", "error", {"command": command})
            )
        outcome = "result" if completed.returncode == 0 else "error"
        content = completed.stdout or completed.stderr or ""
        return _HostResult(
            _bounded(content)
            + (f"\n[exit {completed.returncode}]" if completed.returncode else ""),
            "command",
            self._receipt("bash", outcome, {"command": command}),
        )

    def _read_only_tokens(self, tokens: list[str]) -> bool:
        first = tokens[0].rsplit("/", 1)[-1].casefold()
        if first == "git":
            return len(tokens) >= 2 and tokens[1].casefold() in _READ_ONLY_GIT
    ...
    def websearch(self, query: str) -> _HostResult:
        ...
        try:
            with urllib.request.urlopen(
                f"{engine}?q={urllib.parse.quote(query)}", timeout=8
            ) as response:
    ...
    def test(self, command: str) -> _HostResult:
        ...
        if not tokens or tokens[0].rsplit("/", 1)[-1].casefold() not in {
            "node",
            "pytest",
            "python",
        }:
